    ...
    describe("when I upload an invalid file", function () {
      ...
      test("Bills store should not be updated", function () {
        mockStore
          .bills()
          .list()
          .then((bills) => expect(bills.length).toBe(4));
      });
    });
    ...
    describe("When I submit the form", function () {
      test("Should create a new bill with the inputs values", async function () {
        ...
        await mockStore
          .bills()
          .list()
          .then((bills) => (initialBillsLength = bills.length))
  ...
  describe("When I submit a new bill", () => {
    test("Post data to the mock API", async () => {
      ...
      await mockStore
        ...
        .list()
        .then((bills) => (initialBillsLength = bills.length))
        .then(() => userEvent.click(btn));
      await mockStore
        .bills()
        .list()
        .then((bills) => expect(bills.length).toBe(initialBillsLength + 1));
